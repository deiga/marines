...
BWAPI::TilePosition MarinesAIModule::getBuildLocationNear(BWAPI::TilePosition position, BWAPI::UnitType type)
{
...
bool MarinesAIModule::canBuildHereWithSpace(BWAPI::TilePosition position, BWAPI::UnitType type)
{
  ...
  for(int x = startx; x < endx; x++)
    for(int y = starty; y < endy; y++)
      if (!type.isRefinery())
        if (!buildable(x, y) || reservedMap[x][y])
          return false;
...
bool MarinesAIModule::canBuildHere(BWAPI::TilePosition position, BWAPI::UnitType type)
{
...
bool MarinesAIModule::buildable(int x, int y)
{
...
void MarinesAIModule::onFrame() {
  ...
  if (Broodwar->isReplay()) {
	...
	if (Broodwar->self()->minerals() > 100) {
		...
		BWAPI::TilePosition buildHere = BWAPI::TilePosition(ownStartPosition.x() + 4, ownStartPosition.y() + 3);
		workwork->build(buildHere, UnitTypes::Protoss_Pylon);
...
DWORD WINAPI AnalyzeThread() {
  ...
  if (BWTA::getStartLocation(Broodwar->self())!=NULL) {
	...
	homebase = BWTA::getStartLocation(Broodwar->self());
	ownStartPosition = homebase->getTilePosition();
	Broodwar->printf("X: %d, Y: %d", ownStartPosition.x(), ownStartPosition.y());
  }
...
Position MarinesAIModule::calcEvadePath(int xDir, int yDir, Unit* unit) {
  ...
  if ( unit->getType() == UnitTypes::Protoss_Dragoon ) {
    ...
    xCoord = xDir * (-1) + unit->getPosition().x();
    yCoord = yDir * (-1) + unit->getPosition().y();
  }
